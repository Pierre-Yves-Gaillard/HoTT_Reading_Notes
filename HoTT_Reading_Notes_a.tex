...
\section{The type family \textsf{code} in Section 2.13}

One can prove, at the beginning of \S2.13, that there is a type family $\code$ satisfying 
$$
\code:\N\to\N\to\U,
$$
$$
...
$$
$$
\code(\su(m),0):\equiv\mbf0,
$$
$$
\code(0,\su(n)):\equiv\mbf0,
$$
$$
\code(\su(m),\su(n)):\equiv \code(m,n)
$$
for all $m,n:\N$ as follows:

Define $\code(0):\N\to\U$ by 
$$
\code(0):\equiv\rec_\N(\U,\mbf1,n\mapsto X\mapsto\mbf0).
$$
For $Y:\N\to\U$, define $Y':\N\to\U$ by 
$$
Y':\equiv\rec_\N(\U,\mbf0,n\mapsto X\mapsto Y(n)).
$$
Then $\code:\N\to\N\to\U$ can be defined by 
$$
\code:\equiv\rec_\N(\N\to\U,\code(0),m\mapsto Y\mapsto Y').
$$
