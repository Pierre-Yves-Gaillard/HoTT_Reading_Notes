...
$$ 
hold, and $d(\mbf2,c)$ is ``the'' nonidentity automorphism of $\mbf2$. Then there is a dependent function $f:\prod_{A:\U}A\to A$ such that $f(\mbf2)=d(\mbf2,c)$. %$\prod_{A:\mathcal U}A\to A$ is not contractible.
...
e:\prod_{A:\U}(P(A)+\neg P(A))\to A\to A
$$ 
by $e(A,\inl(x)):\eq d(A,x)$, $e(A,\inr(x)):\eq\id_A$; and define 
$$
f:\prod_{A:\U}A\to A
$$ 
by $f(A):\eq e(A,\LEM(P(A),b(A)))$. The equality $\LEM(P(\mbf2),b(\mbf2))=\inl(c)$, which follows from Exercise~3.6 (see \S\ref{e36} p.~\pageref{e36} above), implies  
$$
f(\mbf2):\eq e(\mbf2,\LEM(P(\mbf2),b(\mbf2))=e(\mbf2,\inl(c))=d(\mbf2,c).\ \square 
$$ 
...
$$ 
Define $Q:\U\to\U$ by 
%$$Q(A):\eq\{f:A\to A\ |\ \isequiv(f)\text{ and }f\neq\id_A\}.$$ 
$$
Q(A):\eq\sum_{f:A\to A}\isequiv(f)\times\neg(f=\id_A).
$$
In particular we have the dependent function 
$$
...
$$
d(A,x):\eq\pr_1(A,\msf{center}(x)).
$$ 
It is straightforward to verify that $(P,b,c,d)$ satisfies the required conditions. $\square$
...
We prove the existence of $u$ and $v$ as follows. Defining $t:\mbb S^{n+1}\to P(h(r))$ as in the book and setting $h'(r,r'):\eq u:\eq t(\msf{base})$, $c_u(x):\eq u$ for all $x:\mbb S^{n+1}$ and 
$$
M:\eq\msf{Map}_*((\mbb S^{n+1},\msf{base}),(P(h(r)),u)),
$$ 
