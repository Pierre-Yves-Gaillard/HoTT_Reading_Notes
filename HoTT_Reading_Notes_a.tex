...
$$=\sum_{p:g(f(x))=g(b)}\ \sum_{q:f(x)=b}\transport^{\lambda y.y=g(b)}(q,p)=\refl_{g(b)}$$ 
...
$$=\sum_{x:A}\ \sum_{y:B}\ (q:g(y)=c)\times(f(x)=y)$$ 
...
$$\psi:(g(f(x))=c)\to\sum_{y:B}\ (q:g(y)=c)\times(f(x)=y)$$ 
