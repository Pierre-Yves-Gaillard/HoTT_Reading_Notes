...
\subsection{Additional exercise to Chapter 3}

The statement of this additional exercise to Chapter~3 is:

Is the type $\prod_{A:\mathcal U}A\to A$ contractible?

Following an argument given by Jason Gross in 
...
$$ 
and $d(\mbf2,c)\neq\id_{\mbf2}$. Then $\prod_{A:\mathcal U}A\to A$ is not contractible.
...
$$
f(\mbf2)\de e(\msf{lem}(P(\mbf2),b(\mbf2))=e(\msf{inl}(c))=d(\mbf2,c)\neq\id_{\mbf2}. 
$$ 
...
\subsection{Proof of Corollary 6.4.3, version 1}
...
Let $A$ and $B$ be types, let $f,g:A\to B$, let $u:\mathsf{isequiv}(f),\ u:\mathsf{isequiv}(g)$, so that $(f,u),(g,v):A\simeq B$. Put $C:\equiv\mathsf{isequiv}(g), D(p):\equiv(p_*(u)=_Cv)$ for all $p:f=g$. By Theorem 2.7.2 we have 
$$
((f,u)=(g,v))\simeq\sum_{p:f=g}D(p).
$$ 
We claim that the first projection 
$$
\pr_1:\sum_{p:f=g}D(p)\to(f=g)
$$ 
is an equivalence. This will imply 
$$
((f,u)=(g,v))\simeq(f=g).
$$ 
By Theorem 4.2.13 and the above reminders, $D(p)$ is contractible for all $p:f=g$, and Lemma 3.11.9 (i) implies that $\pr_1$ is indeed an equivalence.

%%

\subsection{Proof of Corollary 6.4.3, version 2}

This is a second version of the previous subsection. I'm planning keep only one these two subsections in the next versions of this text.
