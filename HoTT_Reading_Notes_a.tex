...
\emph{Statement.} Show that if we define $A+B:\equiv\sum_{x:\mbf2}\rec_\mbf2(\U,A,B,x)$, then we can give a definition of $\ind_{A+B}$ for which the definitional equalities stated in \S1.7 hold.

\nn\emph{Solution.} Recall that we have 
$$
\rec_\mbf2:\prod_{C:\U}C\to C\to\mbf2\to C,\quad\rec_\mbf2(C,c_0,c_1,0_\mbf2)\de c_0,\quad\rec_\mbf2(C,c_0,c_1,1_\mbf2)\de c_1.
$$
We define $\inl:A\to A+B$ by $\inl(a):\equiv(0_\mbf2,a)$, and we define $\inr$ similarly. Let $C:A+B\to\U$ be given, and put 
$$
...
$$ 
Set $T:\equiv\rec_\mbf2(\U,A,B):\mbf2\to\U$. We define $D:\mbf2\to\U$ by 
$$
D(n):\equiv\prod_{u:T(n)}C((n,u)).
$$ 
Note that $D(0_\mbf2)\equiv A'$ and $D(1_\mbf2)\equiv B'$. Recall that 
$$
\ind_\mbf2(D):D(0_\mbf2)\to D(1_\mbf2)\to\prod_{n:\mbf2}D(n).
$$ 
In particular we have 
$$
\ind_\mbf2(D,g_0,g_1):\prod_{n:\mbf2}\ \prod_{u:T(n)}C((n,u)).
$$ 
Since 
$$
\ind_{\sum_{n:\mbf2}T(n)}(C):\left(\prod_{n:\mbf2}\ \prod_{u:T(n)}C((n,u))\right)\to\prod_{x:A+B}C(x),
$$ 
we can put 
$$
\ind_{A+B}(C,g_0,g_1):\equiv\ind_{\sum_{n:\mbf2}T(n)}(C,\ind_\mbf2(D,g_0,g_1)).
$$ 
We must check 
\begin{equation}\label{15a}
\ind_{A+B}(C,g_0,g_1,(0_\mbf2,a))\equiv g_0(a),
\end{equation}
\begin{equation}\label{15b}
\ind_{A+B}(C,g_0,g_1,(1_\mbf2,b))\equiv g_1(b).
\end{equation} 
We have 
\begin{align*}
\ind_{A+B}(C,g_0,g_1,(0_\mbf2,a))&\equiv\ind_{\sum_{n:\mbf2}T(n)}\big(C,\ind_\mbf2(D,g_0,g_1),(0_\mbf2,a)\big)\\ 
&\equiv\ind_\mbf2(D,g_0,g_1)(0_\mbf2)(a)\\ 
&\equiv g_0(a),
\end{align*}
the three definitional equalities following respectively from the definitions of the dependent functions
$$
\ind_{A+B},\quad\ind_{\sum_{n:\mbf2}T(n)},\quad\ind_\mbf2.
$$ 
...
\emph{Statement.} Show that if we define $A\times B:\equiv\prod_{x:\mbf2}\rec_\mbf2(\U,A,B,x)$, then we can give a definition of  $\ind_{A\times B}$ for which the definitional equalities stated in \S1.5 hold propositionally (i.e. using equality types). \emph{(This requires the function extensionality axiom, which is introduced in \S2.9.)}

\nn\emph{Solution.} As above put $T:\equiv\rec_\mbf2(\U,A,B):\mbf2\to\U$, so that $A\times B:\equiv\prod_{n:\mbf2}T(n)$, and 
$$
\ind_\mbf2(T):A\to B\to A\times B.
$$ 
For $a:A,b:B$ we abbreviate $\ind_\mbf2(T,a,b)$ by $(a,b)$; in particular we have 
$$
(a,b):A\times B,\quad(a,b)(0_\mbf2)\equiv a,\quad(a,b)(1_\mbf2)\equiv b.
$$ 
Let $C:A\times B\to\U$. We must define 
$$
\ind_{A\times B}(C):\left(\prod_{a:A}\prod_{b:B}C((a,b))\right)\to\prod_{u:A\times B}C(u).
$$ 
Let $u:A\times B$ and define $D:\mbf2\to\U$ by 
$$
D(n):\equiv\Big(\big(u(0_\mbf2),u(1_\mbf2)\big)(n)=u(n)\Big)
$$ 
and recall that 
$$
\ind_\mbf2(D):D(0_\mbf2)\to D(1_\mbf2)\to\prod_{n\in\mbf2}D(n)
$$ 
and that 
$$
\fu:\left(\prod_{n\in\mbf2}D(n)\right)\to\Big(\big(u(0_\mbf2),u(1_\mbf2)\big)=u\Big).
$$
We get 
$$
\alpha:\equiv\ind_\mbf2(D,\refl_{u(0_\mbf2)},\refl_{u(1_\mbf2)}):\prod_{n\in\mbf2}D(n),\quad\fu(\alpha):\big(u(0_\mbf2),u(1_\mbf2)\big)=u,
$$
$$
\fu(\alpha)_*:C\Big(\big(u(0_\mbf2),u(1_\mbf2)\big)\Big)\to C(u).
$$
In particular we can set
$$
\ind_{A\times B}(C,g,u):\equiv\fu(\alpha)_*\Big(g\big(u(0_\mbf2)\big)\big(u(1_\mbf2)\big)\Big).
$$

To show that the definitional equalities stated in \S1.5 hold propositionally, we assume $u\equiv(a,b)$. In particular we have 
$$
\alpha:\prod_{n\in\mbf2}\Big((a,b)(n)=(a,b)(n)\Big).
$$ 
...
\nn\emph{Solution.} For any types $A,B$ we define $f:\Big(\big(A+(A\to B)\big)\to B\Big)\to B$ by $f(g):\eq(g\ci\inr)(g\ci\inl).$ Putting $B:\equiv\mbf0$ gives the desired result.
...
The equality 
$$
\msf{encode}(\inl(a),\msf{decode}(\inl(a),c))=c
$$ 
...
Let $c,c':\isContr(A)$. Since $A$ is contractible (by $c$ or $c'$), Lemma 3.11.3 implies $A=\mbf1$, and thus 
$$
\isContr(A)=\left(\sum_{a:\mbf1}\prod_{x:\mbf1}a=x\right)=\left(\sum_{a:\mbf1}\prod_{x:\mbf1}\mbf1\right)=\left(\sum_{a:\mbf1}\mbf1\right)=\mbf1.
$$
...
$$
\isProp(A)\de\prod_{a,b:A}a=b,\quad\isContr(A)\de\sum_{a:A}\prod_{b:A}a=b.
$$
...
%We define $u:\isProp(A)\to(A\to\isContr(A))$ by $\big(u(f)(a)\big)(b)\de(a,f(a,b))$.%$u(f)(a)\de(a,f(a,-))$. 

%We define $v:(A\to\isContr(A))\to\isProp(A)$ by $v(g)(a,b)\de\pr_2(g(a)(a)^{-1}\cdot \pr_2(g(a,b))$.

%as follows. Let $g:A\to\isContr(A)$. For any $a:A$ we have $g(a)=(g_1(a),g_2(a))$ with $g_1(a):A$ and $g_2(a):\prod_{b:A}g_1(a)=b$. Thus we can put $v(g)(a,b)\de g_2(a,a)^{-1}\cdot g_2(a,b)$.
...
\nn We leave Cases (a) and (b) to the reader, and take up Case~(c). Let $f:A\times B\to\mbf0$ and $g:\mbf0\to(\inl(a)=\inr(b))$. We get $g(f(a,b)):\inl(a)=\inr(b)$.
...
$$
f_1:\prod_{A:\U}\isProp(A)\to A\to(A=\mbf1). 
$$ 
...
Define 
$$
f_4:\prod_{A:\U}\isProp(A)\to\big((A=\mbf1)+(A=\mbf0)\big)
$$ 
by 
$$
...
$$
f_5:\left(\sum_{A:\U}\isProp(A)\right)\to\mbf2
$$ 
by 
$$
f_5(A,p):\eq
\begin{cases}
1_\mbf2&\text{if }A=\mbf1\\
0_\mbf2&\text{if }A=\mbf0.
\end{cases}
$$ 
Let $p_0:\isProp(\mbf0)$ and $p_1:\isProp(\mbf1)$, and define 
$$
f_6:\mbf2\to\left(\sum_{A:\U}\isProp(A)\right)
$$ 
by $f_6(0_\mbf2):\eq(\mbf0,p_0)$ and $f_6(1_\mbf2):\eq(\mbf1,p_1)$.

It is straightforward to check that $f_5\ci f_6=\id_\mbf2$. 

Let $A:\U$ and $p:\isProp(A)$. We must show 
\begin{equation}\label{f6f5}
f_6(f_5(A,p))=(A,p).
\end{equation}
In view of $f_4$ it suffices to prove that \eqref{f6f5} holds if $A=\mbf1$ or if $A=\mbf0$, which is easy. $\square$ 

Note that the Law of Excluded Middle $\LEM$ is equivalent to 
$$
\LEM':\prod_{A:\U}\isProp(A)\to\big((A=\mbf1)+(A=\mbf0)\big)
$$ 
...
%\subsection{Corollary 4.4.6}Here is a slightly more general statement:\\ To each pair of maps $f:A\to B,P:B\to\U$ is attach a map $$g:\left(\prod_{b:B}P(b)\right)\to\prod_{a:A}P(f(a))$$ defined by $$g(h)(a)\de h(f(a)).$$
...
\end{equation}
Proof: Write $t$ for ``transport''. Set $C\eq\sum_{X:\U}X\times X=A$, and define 
$$
f:A\to C,\qquad g:C\to A
$$ 
by 
$$
f(a):\eq(A,a,\re_A),\qquad g(B,b,p):\eq t^{X\mapsto X}(p,b).
$$ 

\nn We have $g(f(a))\eq a$ for all $a:A$. Let $(B,b,p):C$. We have 
$$
f(g(B,b,p))=(A,t^{X\mapsto X}(p,b),\re_A)
$$ 
and we must show 
$$
(B,b,p)=(A,t^{X\mapsto X}(p,b),\re_A).
$$
By Theorem 2.7.2 it suffices to verify 
$$
t^{X\mapsto X\times X=A}(p,(b,p))=(t^{X\mapsto X}(p,b),\refl_A).
$$ 
...
\nn by $h(x,p)\de(f(x),p)$. (Note that $p:g(f(x))=g(b)$.) We get 
...
$$\ph(y,q,p)\de\ap_g(p)\cdot q,\quad\psi(r):\eq(f(x),r,\refl_{f(x)})$$ 
...
\nn\emph{Solution.} Setting $k\de f\circ (g\circ f)^{-1}$, we get 
\begin{align*}
k&\de f\circ (g\circ f)^{-1}\\
...
\nn$\bullet\ \msf{ppmap}(R,S)$ is inhabited: Setting $D(b,r)\de S(b),d\de s_0$, we get $f:\prod_{b:A}S(b)$ with $f(a_0)=s_0$. Define $g:\prod_{b:A}R(b)\to S(b)$ by $g(b,r)\de f(b)$. This yields $g(a_0,r_0):\equiv f(a_0)=s_0$, as required. 

\nn$\bullet\ \msf{ppmap}(R,S)$ is a mere proposition: Let 
$$
f,g:\prod_{b:A}R(b)\to S(b),\qquad p:f(a_0,r_0)=s_0,\qquad q:g(a_0,r_0)=s_0,
$$ 
so that $(f,p),(g,q):\msf{ppmap}(R,S)$. It suffices to prove $(f,p)=(g,q)$. Setting $D(b,r)\de(f(b,r)=g(b,r)),d\de p\cdot q^{-1}$, we get $h:\prod_{b:A}\prod_{r:R(b)}f(b,r)=g(b,r)$ with $h(a_0,r_0)=p\cdot q^{-1}$. Function extensionality yields a path $t:f=g$ satisfying $t_*(p)=h(a_0,r_0)^{-1}\cdot p=(p\cdot q^{-1})^{-1}\cdot p=q$.%by function extensionality, and finally $t_*(p)=h(a_0,r_0)^{-1}\cdot p=(p\cdot q^{-1})^{-1}\cdot p=q$.%. It suffices to show $t_*(p)=q$; this follows from the equalities $t_*(p)=h(a_0,r_0)^{-1}\cdot p=(p\cdot q^{-1})^{-1}\cdot p$.%, that is $(p\cdot q^{-1})^{-1}\cdot p=q$, which is clear.
...
Recall the statement of the lemma: The type $\mbb S^1$ satisfies $\msf{loop}\neq\refl_{\msf{base}}$. 
...
$$
$$
\gamma:\prod_{c:C}\ \alpha\big(\inl(f(c))\big)=^D_{\msf{glue}(c)}\beta\big(\inr(g(c))\big).
$$ 
Then there is an $h:\prod_{z:Z}D(z)$ such that $h\circ \inl\equiv\alpha$, $h\circ \inr\equiv\beta$ and $\apd_h(\msf{glue}(c))=\gamma(c)$ for all $c:C$. 

%\begin{comment}
%Here are more details about the end of the proof of Lemma 6.8.2:

%Let $u\de\msf s(t\circ c_\sqcup)$, define $D:Z\to\U$ by $D(z)\de(u(z)=t(z))$, define $$\alpha(a):u(\inl(a))=t(\inl(a)),\quad\beta(b):u(\inr(b))=t(\inr(b))$$ for $a:A,\ b:B$ as in the book, and, for $c:C$, let $\gamma(c)$ be the obvious inhabitant of the type $$\alpha(\inl(f(c)))=^D_{\msf{glue}(c)}\beta(\inr(g(c))),$$ which is equivalent to the type $$u(\msf{glue}(c))^{-1}\cdot\alpha(f(c))\cdot t(\msf{glue}(c))=\beta(g(c)).$$ Then the induction principle yields a dependent function $h:\prod_{z:Z}u(z)=t(z)$.
%\end{comment}
...
Assuming the Law of Excluded Middle, construct a family $f:\prod_{A:\U}A\to A$ such that $f(\mbf2):\mbf2\to\mbf2$ is the nonidentity automorphism.
...
\nn\textbf{Claim.} Assume there is a quadruple $(P,b,c,d)$ such that the typing judgments  
$$
P:\U\to\U,\quad b:\prod_{A:\U}\isProp(P(A)),\quad c:P(\mbf2),\quad d:\prod_{A:\U}P(A)\to A\to A
$$ 
hold, and $d(\mbf2,c)$ is ``the'' nonidentity automorphism of $\mbf2$. Then there is a family $f:\prod_{A:\U}A\to A$ such that $f(\mbf2)=d(\mbf2,c)$. %$\prod_{A:\mathcal U}A\to A$ is not contractible.
...
\nn\emph{Proof of the claim.} Define 
$$
e:\prod_{A:\U}(A+\neg A)\to A\to A
$$ 
by $e(A,\inl(x))\de d(A,x)$, $e(A,\inr(x))\de\id_A$; and define 
$$
f:\prod_{A:\U}A\to A
$$ 
by $f(A)\de e(\LEM(P(A),b(A)))$. The equality $\LEM(P(\mbf2),b(\mbf2))=\inl(c)$, which is easily checked, implies  
$$
f(\mbf2)\de e(\LEM(P(\mbf2),b(\mbf2))=e(\inl(c))=d(\mbf2,c).\ \square 
$$ 
%This shows that $\prod_{A:\mathcal U}A\to A$ is not contractible. qed

\nn\emph{Proof of the existence of} $(P,b,c,d)$. By definition of $\isContr$ we have a dependent function 
$$
\msf{center}:\prod_{A:\U}\isContr(A)\to A.
$$ 
Define $Q:\U\to\U$ by 
$$
Q(A)\de\{f:A\to A\ |\ \isequiv(f)\text{ and }\exists\ (a:A)\ .\ f(a)\neq a\}.
$$ 
...
P(A)\de\isContr(Q(A)).
$$ 
The existence of $b$ follows from Lemma 3.11.4. The existence of $c$ is clear. Finally we define $d$ by $d(A,x)\de\pr_1(\msf{center}(x))$. It is straightforward to verify that $(P,b,c,d)$ satisfies the required conditions. $\square$
...
We prove 
$$
f(x,x,\rho(x))\cdot p=f(x,y,\transport^{x=-}(p,\rho(x)))
$$ 
