...
The phrase ``$A$ is $n$-truncated'' means ``$A$ is an $n$-type''. The definition of $n$-truncated map is given in Definition 7.6.1: a map is $n$-truncated if all its fibers are $n$-types.
...
%\subsection{Injectivity of the map from Q to Rd (Section 11.2.1)}

It is asserted just before \S11.2.1 that the map $f:\mbb Q\to\mbb R_d$, $q\mapsto(L_q,U_q)$ is an embedding, but the claim is not proved. The claim results from the following more precise observations, which could be made just before Lemma 11.2.2: 

For $x,y:\mbb R_d$ the inequality $x<y$ implies $x\neq y$. Moreover we have
$$
f(q)<f(r)\iff q<r,\quad f(q)\le f(r)\iff q\le r.
$$ 
...
$$
(\lvert x\rvert<\e)\de(-\e<x)\vee(x<\e).
$$
