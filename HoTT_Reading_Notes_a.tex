...
For $x,y:A$ define 
$$
f(x,y):(x=y)\to(\lvert x\rvert_{n+1}=\lvert y\rvert_{n+1})
$$ 
by 
$$
f(x,y)(p)\de\msf{ap}_{\lvert-\rvert_{n+1}}(p).
$$ 
By recursion we get a section 
$$
g:\prod_{x,y:A}\lVert x=y\rVert_n\to(\lvert x\rvert_{n+1}=\lvert y\rvert_{n+1}),
$$
that is 
$$
g:\prod_{x,y:A}P(\lvert x\rvert_{n+1},\lvert y\rvert_{n+1})\to(\lvert x\rvert_{n+1}=\lvert y\rvert_{n+1}),
$$ 
such that 
$$
g(x,y)(\lvert p\rvert_n)\equiv f(x,y)(p)\equiv\msf{ap}_{\lvert-\rvert_{n+1}}(p)
$$ 
for all $x,y:A$ and all $p:x=y$. By induction on $u,v:\lVert A\rVert_{n+1}$ we get a section 
$$
\msf{decode}:\prod_{u,v:\lVert A\rVert_{n+1}}P(u,v)\to(u=v)
$$ 
such that 
$$
\msf{decode}(\lvert x\rvert_{n+1},\lvert y\rvert_{n+1})\equiv g(x,y)
$$ 
for all $x,y:A$. 
