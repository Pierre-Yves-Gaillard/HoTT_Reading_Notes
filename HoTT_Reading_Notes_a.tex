...
$$
\alp(p,q):\tr^{\hat{\ms{code}}}(p,q)=q\cc p
$$ 
the obvious path in $N=y_2$. We also denote by 
$$
\bet:\ms{merid}(x_0)\cc\ms{merid}(x_0)^{-1}=\refl_N
$$ 
the obvious path in $N=N$. 

The underscores $u_1,\dots,u_8$ can be chosen as follows:
$$
u_1:\eq\alp(\ms{merid}(x_1)\cc\ms{merid}(x_0)^{-1},\refl_N),\quad u_2:\eq u_5:\eq\bet,
$$ 
$$
u_3:\eq u_6:\eq\alp(\ms{merid}(x_0)^{-1},\ms{merid}(x_1)),\quad u_4:\eq\alp(\ms{merid}(x_1),\refl_N),
$$ 
$$
u_7:\eq\refl_{\ms{merid}(x_1)},\quad u_8:\eq\refl_{\ms{merid}(x_1)\cc\ms{merid}(x_0)^{-1}}.
$$
