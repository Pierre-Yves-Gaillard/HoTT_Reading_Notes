...
\end{equation}
Proof: Write $\ms t$ for ``transport''. Set $C:\eq\sum_{X:\U}\big(X\times(X=A)\big)$, and define 
$$
...
\tb{Claim.} \emph{If $n\ge-1$ and if $A$ is a type, then $A$ is an $n$-type if and only if the type 
$$
