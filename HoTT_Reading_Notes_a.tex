...
%\begin{comment} Here is a slightly stronger statement: For any types $A,B,C$ the types
...
%\nn are inhabited. The proof that (a), (b), (c) are inhabited is left to the reader. The fact that (a) and (c) are inhabited implies that (d) and (e), and thus (f), are inhabited. Putting $B:\equiv\mb0$ in (f) gives the desired result. \end{comment}
...
h':(\mb2=A)\to\neg\neg A.
$$ 
To do this, we set $h'(p,k):\eq k(p_*(0_{\mbf2}))$. 
...
$$
by path induction in the obvious way. Let $\mathsf{qinv}$-$\mathsf{univalence}$ denote the modified form of the univalence axiom which asserts that for all $A,B:\U$ the function $\mathsf{idtoqinv}_{A,B}$ has a quasi-inverse.
...
$$ 
For all $A,B:\U$ let $e(A,B):(A=B)\to(A\backsimeq B)$ be the natural map (which we assume to admit a quasi-inverse).
...
Suppose first $<$ is well-founded, and suppose $B:\mc P(A)$ is a %nonempty 
subset with no minimal element. That is, for any $a:A$ with $a\in B$, there merely exists a $b:A$ with $b<a$ and $b\in B$. 

We claim that there is a $g:\prod_{a:A}(a\notin B)$. It suffices to show that there is an 
$$
f:\prod_{a:A}\left(\prod_{a':A}(a'<a)\to(a'\notin B)\right)\to(a\notin B).
$$ 
Let $a:A$ and $s:\prod_{a':A}(a'<a)\to(a'\notin B)$ and assume $a\in B$. It suffices to prove $\mbf0$. As observed above, there is a $b\in B$ and an $l:b<a$, yielding the contradiction $s(b,l):b\notin B$.

%It suffices to prove $\mbf0$. 

%By \S\ref{s1037} above, it suffices to find an $f$ as in \eqref{e1037} with $P(a)\eq\mbf0$ for all $a:A$. Given $a,c:A$ and $k:\prod_{b:A}\neg(b<a)$, it suffices to prove $\mbf0$. Taking $a,b:A$ with $a,b\in B$ and $l:b<a$ as above, we get $k(b,l):\mbf0$. 

Now suppose each nonempty subset merely has a minimal element, and assume that
$$
B:\eq\{a:A\ |\ \neg\acc(a)\}
$$ 
is nonempty. It suffices to prove $\mbf0$. We can assume that a minimal element $a_0$ of $B$ exists purely. %As in the first part of the proof, 
It suffices to find an 
$$
f:\prod_{a:A}\neg\left(\left(\prod_{b:A}(b<a)\to\acc(b)\right)\times\prod_{b:A}\neg(b<a)\right)
$$ 
Let $h:\prod_{b:A}(b<a_0)\to\acc(b)$. It suffices to prove $\mbf0$, which we can do by noting that $\acc_<(h,a_0):\mbf0$. $\square$

%$f$ as in \eqref{e1037} with $P(a)\eq\mbf0$ for all $a:A$. 

%Then if $B$ is nonempty, it merely has a minimal element. Thus there merely exists an $a:A$ with $a\in B$ such that for all $b<a$, we have $\acc(b)$. But then by definition (and induction on truncation), $a$ is merely accessible, and hence accessible, contradicting $a\in B$. Thus, $B$ is empty, so $<$ is well-founded.
