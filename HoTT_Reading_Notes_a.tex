...
$$ 
such that $g(x,x,\re_x)\eq d$ for all $x:A$. We set $f(x,p):\eq g(a,x,p,C_0,c_0)$ for $x:A$ and $p:a=x$, and leave the end of the proof to the reader.
...
$$ 
(see (5.6.7) in the book) if $P:W\to\U$. The solution $s$ to the recurrence $r$ satisfies in both cases 
$$
...
%\subsection[Induction principle for ||A||\_n in \S7.3]{Induction principle for $\lV A\rV_n$ in \S7.3}%\label{69}
...
$$ 
for all $a:A$, all $d:B\to W$ and all $b:B$. 
...
$$ 
In particular, the induction principle reads as follows: Given a type family $P:A/'R\to\U$, we say that a dependent function $f:\pd_{x:A/'R}P(x)$ satifies the recurrences 
$$
r:\pd_{a:A}P(q'(a)),\quad s:\pd_{a,b:A}R(a,b)\to(r(a)=r(b))
$$ 
if we have 
$$
f(q'(a))\eq r(a),\quad \ms{apd}_f(c(a,b,u))=s(a,b,u)
$$ 
for all $a,b:A$ and all $u:R(a,b)$. 
