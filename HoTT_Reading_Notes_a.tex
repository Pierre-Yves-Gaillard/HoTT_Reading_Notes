...
$$
\ap_{\msf{pr}_1}:(u=v)\to(\msf{pr}_1(u)=\msf{pr}_1(v))
$$ 
is an equivalence.
...
\nn(3) To complete the proof of Lemma 4.1.2, we set $f(x):\eq\msf{pr}_1(k(x))$ with $k:\prod_{x:A}B(x)$.
...
%Recall the statement of Theorem 4.4.5: For any $f:A\to B$ we have $\msf{isContr}(f)\simeq\ishae(f)$.
...
%Let $u\de\msf s(t\circ c_\sqcup)$, define $D:Z\to\U$ by $D(z)\de(u(z)=t(z))$, define $$\alpha(a):u(\msf{inl}(a))=t(\msf{inl}(a)),\quad\beta(b):u(\msf{inr}(b))=t(\msf{inr}(b))$$ for $a:A,\ b:B$ as in the book, and, for $c:C$, let $\gamma(c)$ be the obvious inhabitant of the type $$\alpha(\msf{inl}(f(c)))=^D_{\msf{glue}(c)}\beta(\msf{inr}(g(c))),$$ which is equivalent to the type $$u(\msf{glue}(c))^{-1}\cdot\alpha(f(c))\cdot t(\msf{glue}(c))=\beta(g(c)).$$ Then the induction principle yields a dependent function $h:\prod_{z:Z}u(z)=t(z)$.
%\end{comment}
