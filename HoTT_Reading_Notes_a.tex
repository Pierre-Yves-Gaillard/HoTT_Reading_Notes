...
%\begin{align*}\ms a(\ms s(m))&\equiv\ms r(C,\ms s,c_s,\ms s(m))\\&\equiv c_s(m,\ms r(C,\ms s,c_s,m))\\&\equiv c_s(m,\ms a(m))\\&\equiv\ms r(\N,d_0(m,\ms a(m)),d_s(m,\ms a(m))),\end{align*}
...
%\begin{align*}\ms a(m)\big(\ms a(\ms s(m),n)\big)&\equiv\ms a\big(m,\ms a(\ms s(m),n)\big)\\ &\equiv\ms a(\ms s(m),\ms s(n))\\ &\equiv\ms r\big(\N,\ms a(m,1),d_s(m,\ms a(m)),\ms s(n)\big)\\ &\equiv d_s\big(m,\ms a(m)\big)\bigg(n,\ms r\Big(\N,\ms a(m,1),d_s\big(m,\ms a(m)\big),n\Big)\bigg)\\ &\equiv d_s\big(m,\ms a(m)\big)\Big(n,\ms a\big(\ms s(m),n\big)\Big),\end{align*}
...
$$
f:C\to A,\quad g:C\to B,\quad\gl:\pd_{c:C}\ \Big(\inl\big(f(c)\big)=\inr\big(g(c)\big)\Big).
$$ 

Given 
$$
\alpha:A\to D,\quad\beta:B\to D,\quad\gamma:\prod_{c:C}\Big(\alpha\big(f(c)\big)=\beta\big(g(c)\big)\Big),
$$ 
...
$$
\alpha:\prod_{a:A}\Big(s\big(\inl(a)\big)=s'\big(\inl(a)\big)\Big),\quad\beta:\prod_{b:B}\Big(s\big(\inr(b)\big)=s'\big(\inr(b)\big)\Big),
$$
$$
\gamma:\prod_{c:C}\ga(\alpha\big(f(c)\big)=^{z\mapsto(s(z)=s'(z))}_{\glue(c)}\beta\big(g(c)\big)\dr).
$$ 
Then there is an $h:\prod_{z:Z}\big(s(z)=s'(z)\big)$ such that 
$$
