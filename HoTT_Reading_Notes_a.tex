...
Set 
$$
\ms a:\eq\ack,\quad\ms r:\eq\rec_\N,\quad\ms s:\eq\ms s.
$$

We try to solve Equation 
\begin{equation}\label{ack1}
\ms a:\equiv\ms r(C,c_0,c_s)
\end{equation}
for $C:\U,c_0:C,c_s:\N\to C\to C$. We get $C:\equiv(\N\to\N)$, and evaluation at 0 gives $c_0\equiv\ms s$. Let $i:\N,f:\N\to\N$. It suffices to solve Equation 
\begin{equation}\label{ack2}
c_s(i,f):\equiv\ms r(D(i,f),d_0(i,f),d_s(i,f))
\end{equation}
for 
$$
D(i,f):\U,\quad d_0(i,f):D(i,f),\quad d_s(i,f):\N\to D(i,f)\to D(i,f).
$$ 
As we have $c_s(i,f):\N\to\N$ and $\ms r(D(i,f),d_0(i,f),d_s(i,f)):\N\to D(i,f)$, we set $D(i,f):\equiv\N$. We have
\begin{align*}
\ms a(\ms s(m))&\equiv\ms r(C,\ms s,c_s,\ms s(m))\\
&\equiv c_s(m,\ms r(C,\ms s,c_s,m))\\
&\equiv c_s(m,\ms a(m))\\
&\equiv\ms r(\N,d_0(m,\ms a(m)),d_s(m,\ms a(m))),
\end{align*}
and evaluation at 0 yields $d_0(m,\ms a(m))\equiv\ms a(m)(1)$, prompting us to set 
$$
d_0(i,f):\equiv f(1),
$$ 
yielding 
$$
\ms a(\ms s(m))\equiv\ms r\Big(\N,\ms a(m,1),d_s\big(m,\ms a(m)\big)\Big).
$$
We have 
\begin{align*}
\ms a(m)\big(\ms a(\ms s(m),n)\big)&\equiv\ms a\big(m,\ms a(\ms s(m),n)\big)\\ 
&\equiv\ms a(\ms s(m),\ms s(n))\\ 
&\equiv\ms r\big(\N,\ms a(m,1),d_s(m,\ms a(m)),\ms s(n)\big)\\
&\equiv d_s\big(m,\ms a(m)\big)\bigg(n,\ms r\Big(\N,\ms a(m,1),d_s\big(m,\ms a(m)\big),n\Big)\bigg)\\
&\equiv d_s\big(m,\ms a(m)\big)\Big(n,\ms a\big(\ms s(m),n\big)\Big),
\end{align*}
prompting us to set $d_s(i,f)(x,y):\equiv f(y)$. 

Abbreviating $d_s(i,f)$ by $f'$, we get 
$$
\ms a:\equiv\ms r(\N\to\N,\ms s,c_s)
$$ 
with 
$$ 
c_s(i,f)\equiv\ms r(\N,f(1),f')
$$ 
for 
$$ 
...
$$
\ms a:\eq\ms r\Big(\N\to\N,\ms s,\la i.\la f.\ms r\big(\N,f(1),\la x.\la y.f(y)\big)\Big).\ \sq
$$
...
\nn\tb{(1)} The fact that $(\star)$ implies that $B(x)$ is a mere proposition follows from Theorem 2.7.2. 
...
\nn\tb{(3)} To complete the proof of Lemma 4.1.2, we set $f(x):\eq\pr_1(k(x))$ with $k:\prod_{x:A}B(x)$. $\sq$
...
$$
C:\equiv\sum_{b:B}\oo{fib}_f(b),
$$ 
and let $\pr_1:C\to B$ be the first projection. We must show 
\begin{equation}\label{2}
(C,\pr_1)=_{\sum_{X:\U}X\to B}(A,f).
\end{equation} 
We have $e:C\simeq A$ by Lemma 4.8.2 (see \S\ref{482} above). Put $q:\equiv\ua(e)$. We have $q:C=A$. We claim 
\begin{equation}\label{3}
q_\star(\pr_1)=f.
\end{equation} 
By Theorem 2.7.2, \eqref{3} will imply \eqref{2}. For all $X:\U$ set $I(X)\equiv X,\ D(X)\equiv B$. Then \eqref{3} becomes, in the notation of (2.9.4), 
$$
\ms t^{I\to D}(q,\pr_1)=f.
$$ 
...
We get 
$$
\ms t^{I\to D}(q,\pr_1,a)\overset{\text{(a)}}=\ms t^D(q,\pr_1(\ms t^I(q^{-1},a)))\overset{\text{(b)}}=\pr_1(e^{-1}(a))\overset{\text{(c)}}=f(a),
$$ 
where (a) follows from \eqref{2.9.4}, (b) follows from the computation rule for univalence stated right after Remark 2.10.4, and (c) follows from the definition of $e^{-1}$. $\sq$
...
$$ 
the first equivalence following from Lemma 4.8.2 (see \S\ref{482}), the second one from \eqref{asimeq}, and the third one being straightforward. $\sq$
...
$$\left(\sum_{w:\fib_g(c)}\fib_f(\pr_1w)\right)=\left(\sum_{x:A}\ g(f(x))=c\right)=\fib_{g\circ f}(c).\ \sq$$
...
%by function extensionality, and finally $t_\star(p)=h(a_0,r_0)^{-1}\cdot p=(p\cdot q^{-1})^{-1}\cdot p=q$.%. It suffices to show $t_\star(p)=q$; this follows from the equalities $t_\star(p)=h(a_0,r_0)^{-1}\cdot p=(p\cdot q^{-1})^{-1}\cdot p$.%, that is $(p\cdot q^{-1})^{-1}\cdot p=q$, which is clear.
