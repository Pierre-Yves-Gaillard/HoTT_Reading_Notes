...
$$
\rec_\mbf2:\prod_{C:\U}C\to C\to\mbf2\to C,\quad\rec_\mbf2(C,c_0,c_1,0_\mbf2):\eq c_0,\quad\rec_\mbf2(C,c_0,c_1,1_\mbf2):\eq c_1.
$$
...
and that 
$$
\funext:\left(\prod_{n\in\mbf2}D(n)\right)\to\Big(\big(u(0_\mbf2),u(1_\mbf2)\big)=u\Big).
$$
We get 
$$
\alpha:\equiv\ind_\mbf2(D,\refl_{u(0_\mbf2)},\refl_{u(1_\mbf2)}):\prod_{n\in\mbf2}D(n),\quad\funext(\alpha):\big(u(0_\mbf2),u(1_\mbf2)\big)=u,
$$
$$
\funext(\alpha)_*:C\Big(\big(u(0_\mbf2),u(1_\mbf2)\big)\Big)\to C(u).
$$
In particular we can set
$$
\ind_{A\times B}(C,g,u):\equiv\funext(\alpha)_*\Big(g\big(u(0_\mbf2)\big)\big(u(1_\mbf2)\big)\Big).
$$
...
Firstly we rewrite (2.9.4) as follows. Abbreviating transport by $\msf t$ we have 
\begin{equation}\label{2.9.4}
\msf t^{A\to B}(p,f,a_2)=\msf t^B(p,f(\msf t^A(p^{-1},x)))
\end{equation}
...
$$
\msf t^{I\to C}(q,\pr_1)=f.
$$ 
Let $a:A$. We must show 
$$
\msf t^{I\to C}(q,\pr_1,a)=f(a).
$$ 
We get 
$$
\msf t^{I\to C}(q,\pr_1,a)\overset{\text{(a)}}=\msf t^C(q,\pr_1(\msf t^I(q^{-1},a)))\overset{\text{(b)}}=\pr_1(e^{-1}(a))\overset{\text{(c)}}=f(a),
$$ 
...
\end{equation}
Proof: Write $\msf t$ for ``transport''. Set $C\eq\sum_{X:\U}X\times X=A$, and define 
$$
f:A\to C,\qquad g:C\to A
$$ 
by 
$$
f(a):\eq(A,a,\re_A),\qquad g(B,b,p):\eq\msf t^{X\mapsto X}(p,b).
$$ 

\nn We have $g(f(a))\eq a$ for all $a:A$. Let $(B,b,p):C$. We have 
$$
f(g(B,b,p))=(A,\msf t^{X\mapsto X}(p,b),\re_A)
$$ 
and we must show 
$$
(B,b,p)=(A,\msf t^{X\mapsto X}(p,b),\re_A).
$$
By Theorem 2.7.2 it suffices to verify 
$$
\msf t^{X\mapsto X\times X=A}(p,(b,p))=(\msf t^{X\mapsto X}(p,b),\refl_A).
$$ 
By Theorem 2.6.4 it suffices to prove 
$$
\msf t^{X\mapsto X=A}(p,p)=\refl_A.
$$ 
...
%Let $u:\eq\msf s(t\circ c_\sqcup)$, define $D:Z\to\U$ by $D(z):\eq(u(z)=t(z))$, define $$\alpha(a):u(\inl(a))=t(\inl(a)),\quad\beta(b):u(\inr(b))=t(\inr(b))$$ for $a:A,\ b:B$ as in the book, and, for $c:C$, let $\gamma(c)$ be the obvious inhabitant of the type $$\alpha(\inl(f(c)))=^D_{\glue(c)}\beta(\inr(g(c))),$$ which is equivalent to the type $$u(\glue(c))^{-1}\cdot\alpha(f(c))\cdot t(\glue(c))=\beta(g(c)).$$ Then the induction principle yields a dependent function $h:\prod_{z:Z}u(z)=t(z)$.
%\end{comment}
...
Assuming the Law of Excluded Middle, construct a dependent function $f:\prod_{A:\U}A\to A$ such that $f(\mbf2):\mbf2\to\mbf2$ is the nonidentity automorphism.
...
We give two formulations of an argument described by Jason Gross in 
...
Let $A$ be a type. We must define $f(A):A\to A$. Set 
$$
C:\eq\sum_{f:A\to A}\isequiv(f)\times\neg(f=\id_A).
$$ 
In particular we have the first projection $\pr_1:C\to A\to A$. Since $\isContr(C)$ is a mere proposition by Lemma 3.11.4, and since we admit the law of excluded middle, we can assume that $C$ is contractible, or that $C$ is not contractible. If $C$ is not contractible, we set $f(A):\eq\id_A$. If $C$ is contractible, we pick an inhabitant of $\isContr(C)$, take its center $c:C$, and set $f(A):\eq\pr_1(c)$. It is straightforward to check that this $f$ does the job.

\subsubsection{Second formulation}

\textbf{Claim.} Assume there is a quadruple $(P,b,c,d)$ such that the typing judgments  
$$
P:\U\to\U,\quad b:\prod_{A:\U}\isProp(P(A)),\quad c:P(\mbf2),\quad d:\prod_{A:\U}P(A)\to A\to A
$$ 
hold, and $d(\mbf2,c)$ is a nonidentity automorphism of $\mbf2$. Then there is a dependent function $f:\prod_{A:\U}A\to A$ such that $f(\mbf2)=d(\mbf2,c)$. 
...
e:\prod_{A:\U}\big(P(A)+\neg P(A)\big)\to A\to A
$$ 
by $e(A,\inl(p)):\eq d(A,p)$ for $p:P(A)$ and $e(A,\inr(p')):\eq\id_A$ for $p':\neg P(A)$; and define 
$$
f:\prod_{A:\U}A\to A
$$ 
by 
$$
f(A):\eq e\Big(A,\LEM\big(P(A),b(A)\big)\Big).
$$ 
The equality 
$$
\LEM\big(P(\mbf2),b(\mbf2)\big)\ =_{P(\mbf2)+\neg P(\mbf2)}\ \inl(c),
$$ 

\nn which follows from Exercise~3.6 (see \S\ref{e36} p.~\pageref{e36} above), implies  

$$
f(\mbf2):\eq e\Big(\mbf2,\LEM\big(P(\mbf2),b(\mbf2)\big)\Big)=e(\mbf2,\inl(c))=d(\mbf2,c).\ \square 
$$ 

\nn\emph{Proof of the existence of} $(P,b,c,d)$. By definition of $\isContr$ we have a dependent function 
$$
\centre:\prod_{A:\U}\isContr(A)\to A.
$$ 
Define $Q:\U\to\U$ by 
$$
Q(A):\eq\sum_{f:A\to A}\isequiv(f)\times\neg(f=\id_A).
$$
In particular we have a dependent function 
$$
\pr_1:\prod_{A:\U}Q(A)\to A\to A.
$$ 
We define $P(A)$ by $P(A):\eq\isContr(Q(A))$. The existence of $b$ follows from Lemma 3.11.4. The existence of $c$ is clear. Finally we define $d$ by 
$$
d(A,p):\eq\pr_1\big(A,\centre(Q(A),p)\big)
$$ 
for $p:P(A)$. It is straightforward to verify that $(P,b,c,d)$ satisfies the required conditions. $\square$
