...
Step 2. Since $\varphi$ is invertible (see Example 2.4.8 in the book), $\ap_g\circ\ap_f$ is also invertible, and so is, by symmetry, $\ap_f\circ\ap_g$. Exercise 4.5 (see \S\ref{45} p.~\pageref{45}) implies that $\ap_f$ and $\ap_g$ are invertible. $\sq$
...
\end{equation}
defined by $f\mapsto(\pr_1\circ f,\pr_2\circ f)$.
...
Now we have to verify that $\chi\ci\psi\simeq\id$ and that $\psi\circ\chi\simeq\id$. 
