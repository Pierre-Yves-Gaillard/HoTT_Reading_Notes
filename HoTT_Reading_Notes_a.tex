...
It suffices to define, keeping $a_0:A$ fixed, the object $h(a_0,\beta)$ for all $\beta:\lV B(a_0)\rV$. Using the notation introduced after Lemma 10.1.6 of the book, set 
$$
C:\eq\{q(a_0,b)\ \vert\ b:B(a_0)\}.
$$ 
It is easy to see that $C$ is a mere proposition, and that we have natural maps
$$
f:B(a_0)\to C,\quad i:C\to\left(\sum_{a:A}B(a)\right)\!\!\!\Bigg/\!\!\!\!\sim
$$ 
which satisfy $i(f(b))\eq q(a_0,b)$ for all $b:B(a_0)$. By \S\ref{69} above, $f$ induces a map $g:\lV B(a_0)\rV\to C$ satisfying $g(\lv b\rv)\eq f(b)$ for all $b:B(a_0)$. Now we can set $h(a_0,\beta):\eq i(g(\beta))$ for all $b:B(a_0)$.
