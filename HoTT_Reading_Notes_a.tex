...
\nn$\bullet\ \msf{ppmap}(R,S)$ is a mere proposition: Let $(f,p),(g,q):\msf{ppmap}(R,S)$. We must prove $(f,p)=(g,q)$. Setting $D(b,r)\de(f(b,r)=g(b,r)),d\de p\cdot q^{-1}$, we get $h:\prod_{b:A}\prod_{r:R(b)}f(b,r)=g(b,r)$ with $h(a_0,r_0)\equiv p\cdot q^{-1}$, and thus $t:f=g$, by function extensionality. It suffices to show $t_*(p)=q$, that is $(p\cdot q^{-1})^{-1}\cdot p=q$, which is clear.
...
Let $u\de\msf s(t\circ c_\sqcup)$, define $D:Z\to\U$ by $D(z)\de(u(z)=t(z)$, set 
$$
...
$$
\Big(\alpha(\msf{inl}(f(c)))=^D_{\msf{glue}(c)}\beta(\msf{inr}(g(c)))\Big)
$$ 
$$ 
=\Big(u(\msf{glue}(c))^{-1}\cdot\alpha(\msf{inl}(f(c)))\cdot t(\msf{glue}(c))=\beta(\msf{inr}(g(c)))\Big).
$$ 
Then the induction principle yields $h:\prod_{z:Z}u(z)=t(z)$.
