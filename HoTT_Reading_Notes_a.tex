...
\end{itemize}
there exists $f:\prod_{x:\lVert A\rVert}B(x)$ such that $f(\lvert a\rvert)\equiv g(a)$ for $a:A$, and also another computation rule. However, because there can be at most one function between any two mere propositions (up to homotopy), this induction principle is not really useful (see also Exercise 3.17).''
...
It is easy to see that if our type family $P:A/R\to\U$ is such that $P(q(b))$ is a mere proposition whenever $R(a,b)$, and if we are just given an $f:\prod_{a:A}P(q(a))$, then we get a $g:\prod_{x:A/R}P(x)$ such that $g(q(a))=^P_{\alpha(a,b)}g(q(b))$ for all $a,b$ satisfying $R(a,b)$.
...
\left(\sum_{a:A}B(a)\right)\!\!\!\Bigg/\!\!\!\!\sim\ \ =\sum_{a:A}\ \lV B(a)\rV.
$$%\end{equation}

To prove this, it suffices, by Lemma 4.8.2 of the book, to note that the left and right hand sides have the same induction principle (see \S\S\ref{69}, \ref{610} above, and Theorem 7.3.2 of the book). $\square$
