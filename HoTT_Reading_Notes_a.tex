...
Suppose first $<$ is well-founded and $A$ has no minimal element. That is, for any $a:A$ there merely exists a $b:A$ with $b<a$. 

We claim that there is a $g:\prod_{a:A}\mbf0$, that is $A=\mbf0$. It suffices to show that there is an 
$$
f:\prod_{a:A}\left(\prod_{a':A}(a'<a)\to\mbf0\right)\to\mbf0.
$$ 
Let $a:A$ and $s:\prod_{a':A}(a'<a)\to\mbf0$. It suffices to prove $\mbf0$. As observed above, there is a $b:A$ and an $l:b<a$, yielding $s(b,l):\mbf0$.
