...
We must define $f(A):A\to A$. Since we have 
$$
g:\prod_{B:\U}\isProp(\isContr(B)),
$$ 
by Lemma 3.11.4, it suffices to define a map $h:\isProp(\isContr(C))\to A\to A$. Since we have 
$$
...
$$
h_1:\isContr(C)\to A\to A\quad\text{and}\quad h_0:\neg\isContr(C)\to A\to A.
$$ 
We set $h_0(x):\eq\id_A$ for all $x:\neg\isContr(C)$, and, for all $x:\isContr(C)$, we let $h_1(x)$ be the first projection of the center of $x$. Finally we set 
$$
f(A):\eq h(\LEM(g(C))).
$$
It is straightforward to check that this definition of $f(A)$ does the job. $\square$
